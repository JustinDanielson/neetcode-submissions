    ...
    def _binary_search(self, key, timestamp) -> int:
        ...
        while l < r:
            if records[m][TimeMap.TS_IDX] > timestamp:
                ...
                # if this timestamp is == the parm timestamp, then
                # the next loop will reduce r to m - 1 and the loop will terminate
                # l = idx+1, r=idx, then m = (l + r) // 2 will truncate to the correct idx
                l = m + 1
            m = (l + r) // 2
        ...
        if len(records) == 0:
            return -1
        if records[m][TimeMap.TS_IDX] > timestamp:
            # second cond handles edge case of 1 item in record with greater ts
            return m - 1
        else:
            return m
